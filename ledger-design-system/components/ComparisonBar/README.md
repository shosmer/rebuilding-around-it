...
- Panel: `surface-1`, `stroke-hair` edge in `line-1`, `radius-lg`, padding `space-6`.
...
- Bars: `bar-thickness` tall, square at the baseline, `radius-sm` on the free end. This year is `series-1`, prior year `series-2`, and when `highlight` names a row every other this-year bar is `series-muted`. Stacked or adjacent bars are separated by a `space-0.5` gap of `surface-1`.
- Track: the full width in `surface-2` so a small value still reads against something.
